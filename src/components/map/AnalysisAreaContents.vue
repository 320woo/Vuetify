    ...
    methods: {
      ...
      deleteAreaContent() {
        if (this.deleteAreaDialog) {
          this.deleteAreaDialog = false
          this.removeAnalysisArea(this.id)
          this.requestAnalysisArea()
        }
      },

      getContentTitle(content) {
        if (content !== undefined) {
          let index = this.getAnalysisAreaList.findIndex((data) => {
      ...
      setContent() {
        let areaList = this.getAnalysisAreaList
      ...
      async requestAnalysisArea() {
        let area = this.getAnalysisAreaList
        console.log('[requestAnalysisArea] this.analysisId : ' + this.analysisId + ', area : ' + area)

        let analysis = StringUtil.clone(Constants.REQUEST_ANALYSIS_PARAM)
        analysis.spray_area = area.map((data) => {
        ...
        let result = await dbAPI.query('update.analyze_opinion.area', {
          'analyze_id': this.analysisId,
          'analyze_area': analysis
        })

        console.log('[requestAnalysisArea] result : ', result)
      }
    },
    computed: {
      ...mapGetters([
