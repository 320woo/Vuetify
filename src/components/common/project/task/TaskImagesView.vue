        ...
        <template slot="items" slot-scope="props">
          ...
          <td class="text-xs-right justify-end pr-3">
            <v-btn icon
                   class="ma-0"
                   ripple
                   @click.prevent="_onClickImage(props.item.file_id)">
              <v-icon color="grey"
                      dark
                      small>
                panorama
...
<script>
  ...
  export default {
    ...
    methods: {
      ...
      async _requestGetFileList({projectId, taskId}) {
        ...
        // this.imageList = await this.task.imagelist
      ...
      _onClickImage(fid) {
        this.imageIndex = this.imageList.findIndex(image => {
          console.log(image)
          return image.file_id === fid;
        })
